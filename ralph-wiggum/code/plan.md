...
| [x] | **Task 11**: Create `DeleteBoardDialog` component | `specs/4.3-ui-components.md:L28-37` | Props: `boardId`, `projectId`, `open`, `onOpenChange`. Warning message, redirect if deleting active board. Done in v0.3.79 |
| [x] | **Task 12**: Create `RenameBoardDialog` component | `specs/4.3-ui-components.md:L40-44` | Props: `boardId`, `currentName`, `open`, `onOpenChange`. Input with current name, calls `useUpdateBoard`. Done in v0.3.80 |
| [x] | **Task 13**: Create `BoardActionsMenu` component | `specs/4.3-ui-components.md:L19-25` | Props: `boardId`, `boardName`, `projectId`. DropdownMenu with Rename/Delete items, orchestrates dialogs. Done in v0.3.81 |
| [x] | **Task 14**: Create barrel export `features/boards/index.ts` | - | Export all components and hooks for clean imports. Done in v0.3.81 |
...
| [x] | **Task 16**: Add loading, error, and empty states for boards list | `specs/4.4-integration.md:L23-25` | Already implemented in Task 15: Skeleton loading (2 items), error message, "No boards yet" placeholder. Done in v0.3.83 |
| [x] | **Task 17**: Wire up `CreateBoardDialog` and `BoardActionsMenu` in sidebar | `specs/4.4-integration.md:L17-19` | Create button in group action, actions menu on each board item, active highlighting. Done in v0.3.84 |
...
| Status | Task | Spec Reference | Notes |
|--------|------|----------------|-------|
| [x] | **Task 18**: Full verification and ROADMAP update | `specs/4.4-integration.md:L27-33` | Run test suite (130 pass), build clean, ROADMAP.md updated. Done in v0.3.84 |
